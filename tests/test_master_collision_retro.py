...
u_sem, t_sem = timed_run("PURE SEM", lambda: jax.lax.scan(step_sem, jnp.copy(u_init_sem), None, length=steps)[0])
...
u_fv, t_fv = timed_run("FV PROXY", lambda: jax.lax.scan(lambda u, _: (run_fv_proxy(u, cx, cy, dt, dx, dx), None), jnp.copy(u_init), None, length=steps)[0])
...
plot_sfx_dashboard(L, X_fft, u_init, u_exact, u_fft, X_sem, u_sem, u_exact_sem, u_hyb_std, u_hyb_sinc, u_fv, err_fft, err_sem, err_hyb_std, err_hyb_sinc, err_fv, t_fft, t_sem, t_hyb_std, t_hyb_sinc, t_fv)
