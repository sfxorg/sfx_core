# 09_SFX_8Panel_POD_Schwarz_V11_1.py
...
PDF = "09_SFX_8Panel_POD_Schwarz_V11_1.pdf"
...
def schwarz_step(P, tol):
    ...
    return (
        split_panels(q),
        np.mean(rank_h) if rank_h else 0,
        np.mean(rank_v) if rank_v else 0,
        residuals[-1] if residuals else 0
    )
...
with PdfPages(PDF) as pdf:
    ...
    for tol in TOLS:
        ...
        ax[1].plot(hranks)
        ax[1].set_title("Horizontal POD Rank")

        ax[2].plot(vranks)
        ax[2].set_title("Vertical POD Rank")
    ...
    lines = [
        "SFX V11.1 8-PANEL POD SCHWARZ",
        "",
        "Tol FinalErr HRank VRank FinalResidual",
        ""
    ]
